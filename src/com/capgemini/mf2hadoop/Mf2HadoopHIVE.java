...
//		String outputFileName=args[6];
		String tablelocation=args[5];
		boolean partitioned_by=args[6].trim().equalsIgnoreCase("Y")?true:false;
		String partition_specification="";
			if(partitioned_by)
				partition_specification=args[7];
		...
		StringBuffer hiveCreateTableQuery= hiveCreateTableGenerator.generateCreateTableQuery(fieldList, tableName, comment, rowFormat, fieldsTerminatedBy, linesTerminatedBy, storedAs,tablelocation, partitioned_by, partition_specification);
